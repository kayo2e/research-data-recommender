...
def build_user_prompt(rec: Dict[str, Any], max_chars: int) -> str:
    # 고시금액·수의계약 기준금액·공동수급 지분율·중기간 경쟁제품 여부 같은 외부 임계값은 모델이
    # "기억해서" 맞히게 두지 않고, 여기서 코드로 미리 계산해 사실 문장으로 준다(thresholds.py).
    threshold_notes = thresholds.build_threshold_notes(rec.get("meta", {}) or {}, DATA_DIR)
    notes_block = f"[계산된 참고 사실]\n{threshold_notes}\n\n" if threshold_notes else ""
    return (
        f"[공고 ID] {rec['id']}\n\n"
        f"[나라장터 입력 메타]\n{format_meta(rec)}\n\n"
        f"{notes_block}"
